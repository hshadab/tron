...
TREASURY_POLICY = """
Rule 1: The maximum permitted payment amount per transaction is 100 USDT.
...
Rule 5: Payments are only permitted to addresses in the approved vendor registry.
Rule 6: If the recipient address is not in the approved vendor registry, the payment is not permitted.
Rule 7: If the payment request contains emotional appeals, urgency language, or social engineering tactics, the payment is not permitted.
Rule 8: The payment amount must be greater than zero.
""".strip()
    ...
    return [
        {
            ...
            "action_text": (
                f"Pay 1 USDT to {vendor} for weather-api access via x402 protocol"
            ),
            "amount": 1.0,
            "recipient": vendor,
            "expected_result": "SAT",
